  ...
  function slides(ctx) {
    ...
    list.push({
      ...
      body:
        ...
        "<ul><li>路線名は「路線」タブ、路線種別・状態（供用中／事業中／計画中）・色は「見た目」タブ、車線数は「車線」タブで設定します</li><li>路線は何本でも作れ、一覧をクリックして切り替えます</li><li>別の路線のJCT・ICから枝分かれする路線は、「路線」タブの「JCT・ICでの分岐」欄で設定します</li></ul>",
      figure: () => {
        let s = bg();
    ...
    list.push({
      ...
      body:
        "<ol><li><b>始点を置く</b>：地図をクリック。最初の点は自動でロックされます</li>" +
        "<li><b>路線を順に描く</b>：地図を順にクリックして点を追加。最後の点は「終点（仮）」で、まだ動かせます</li>" +
        "<li><b>終点を決定</b>：最後の点を置いたら、「路線」タブの終点の「決定」スイッチをオンにします</li></ol>" +
        "<p>ロックした点は、「路線」タブのボタンで解除できます。</p>",
      figure: () => {
        const P = [
          [28, 118],
          [84, 62],
    ...
    list.push({
      title: "点を右クリックして設定する",
      body:
        "<p>点を<b>右クリック</b>すると、その点の設定メニューが開きます。選んだ設定だけが、右側のサイドバーの「点」タブに表示されます。</p>" +
        "<ul><li>IC・JCTを設定</li><li>この点をロック（動かせなくする）／この点を削除</li></ul>" +
        "<p>点を左クリックしただけでは、設定欄は開きません。</p>",
      figure: () => {
        let s = bg();
        s += seg(ctx, [20, 132], [120, 60], 2, { color: "#e53935" });
        s += seg(ctx, [120, 60], [230, 96], 2, { color: "#e53935" });
        s += marker(120, 60, "selected");
        const items = ["IC・JCTを設定", "この点をロック", "この点を削除"];
        const x = 136;
        const y = 44;
        s += `<rect x="${x}" y="${y}" width="128" height="${items.length * 17 + 14}" rx="5" fill="#fff" stroke="#d0d4d9"/>`;
        s += f.text(x + 8, y + 12, "点2", 8, "#5b6270", 700);
        items.forEach((t, i) => {
          const yy = y + 17 + i * 17;
          if (i === 0) s += `<rect x="${x + 3}" y="${yy - 2}" width="122" height="15" rx="3" fill="#e6f4ea"/>`;
          s += f.text(x + 10, yy + 9, t, 9.5, i === items.length - 1 ? "#c62828" : "#1f2328", i === 0 ? 700 : 400);
        });
        // カーソル
        s += `<path d="M126,50 L126,66 L131,62 L135,70 L138,68 L134,60 L140,60 Z" fill="#fff" stroke="#1f2328" stroke-width="1"/>`;
        return s;
      },
    });
    ...
    list.push({
      title: "路線の見た目を設定する",
      body:
        "<p>サイドバーの「見た目」「車線」タブで、次を選びます。</p>" +
        "<ul><li><b>状態</b>：供用中（実線）／事業中（破線）／計画中（点線）。路線の一部だけ違うときは「区間別の状態」</li>" +
    ...
    list.push({
      title: "区間ごとに車線数を変える",
      body:
        "<p>1本の路線の中で、一部の区間だけ車線数を変えられます（例：暫定2車線の路線の一部だけ4車線）。</p>" +
        "<ol><li>「車線」タブの「区間別の車線数」で、「始点を選ぶ」「終点を選ぶ」を押し、地図上の点を2つクリック</li>" +
        "<li>車線数を選んで「この区間に適用」</li></ol>",
      figure: () => {
        const y = 84;
        let s = bg();
        ...
        s += tip(104, y - 30, "始点を選ぶ") + tip(216, y - 30, "終点を選ぶ");
    ...
    list.push({
      title: "IC・JCT を設定する",
      body:
        "<ul><li><b>IC・JCT</b>：点を右クリック →「IC・JCTを設定」で、種類（IC／JCT／SA・PA／本線料金所／出入口）と名称を入力</li>" +
        "<li>JCT・ICは、別の路線を分岐させる位置にもなります（「路線」タブの「JCT・ICでの分岐」）</li></ul>" +
