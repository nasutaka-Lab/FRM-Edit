  ...
  function slides(ctx) {
    ...
    list.push({
      ...
      body:
        ...
        "<ul><li>路線名は「路線」タブ、路線種別・状態（供用中／事業中／計画中）・色・車線数は「設定」タブで設定します</li><li>路線は何本でも作れ、一覧をクリックして切り替えます</li><li>別の路線の施設から枝分かれする路線は、「路線」タブの「JCT・ICでの分岐」欄で設定します</li></ul>",
      figure: () => {
        let s = bg();
    ...
    list.push({
      title: "点を選んで設定する",
      body:
        "<p>点を<b>クリック</b>すると選ばれ、右側のサイドバーの「設定」タブに<b>施設設定</b>欄が開きます（IC・JCTなどを設定できます）。</p>" +
    ...
    list.push({
      title: "区間ごとに車線数を変える",
      body:
        "<p>1本の路線の中で、一部の区間だけ車線数を変えられます（例：暫定2車線の路線の一部だけ4車線）。状態・色も、同じ操作で区間ごとに変えられます。</p>" +
        "<ol><li>「設定」タブの「区間別の設定」で、「始点を選ぶ」「終点を選ぶ」を押し、地図上の点を2つクリック</li>" +
        "<li>車線数（や状態・色）を選んで「この区間に適用」</li></ol>",
      figure: () => {
        const y = 84;
        let s = bg();
    ...
    list.push({
      title: "IC・JCT を設定する",
      body:
        "<ul><li><b>施設設定</b>：点を選ぶと、「設定」タブに開きます。種類（IC／JCT／SA・PA／本線料金所／出入口／その他）と名称を入力</li>" +
        "<li>施設は、別の路線を分岐させる位置にもなります（「路線」タブの「JCT・ICでの分岐」）</li></ul>" +
